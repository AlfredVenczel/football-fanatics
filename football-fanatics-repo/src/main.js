...
const seasonLabel = season => String(season || '').replace('-', '/')
...
function seasonNames() {
  return [...new Set([...Object.keys(standings), ...seasons.map(row => row.season), ...playerSeasons.map(row => row.season), ...playerMatches.map(row => row.season)].filter(Boolean))].sort().reverse()
}
...
function playerMatchSeason(row) { return row.season || '' }
...
function seasonPlayerRows(season) {
  const rows = playerMatches.filter(row => playerMatchSeason(row) === season)
...
function playerProfile(id) {
  ...
  const playerSeasonNames = [...new Set([...rows.map(row => row.season), ...playerSeasons.filter(row => row.player_id === id).map(row => row.season)].filter(Boolean))].sort().reverse()
...
function renderPlayerHistory(rows, season) {
  const data = rows.filter(row => season === 'all' || row.season === season)
