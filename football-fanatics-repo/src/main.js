...
function renderShell() {
  app.innerHTML = `<div class="shell"><aside class="sidebar"><div class="brand"><img class="brand-logo" src="/football-fanatics-logo.webp" alt="Football Fanatics logo"><div><strong>Football Fanatics</strong><small>team hub</small></div></div><nav><button data-page="dashboard" class="active">Tablazat</button><button data-page="matches">Merkozesek</button><button data-page="players">Jatekosok</button></nav><div class="side-foot"><span>${esc(session.user.email)}</span><b>${canEdit() ? 'Editor' : 'Viewer'}</b><button id="signout" class="ghost">Sign out</button></div></aside><main class="main"><header><div class="header-brand"><img class="header-logo" src="/football-fanatics-logo.webp" alt="Football Fanatics logo"><div><span class="eyebrow">Authenticated workspace</span><h1 id="page-title">Tablazat</h1></div></div><span class="role">${canEdit() ? 'EDIT ACCESS' : 'VIEW ONLY'}</span></header><section id="content"></section></main></div>`
